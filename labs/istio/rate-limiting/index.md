...
```sh
kubectl apply -f manifests/local-ratelimit/local-ratelimit.yaml
```
...
```sh
kubectl apply -f manifests/global-ratelimit/global-ratelimit-config.yaml
kubectl apply -f manifests/global-ratelimit/global-ratelimit-service.yaml
```
...
using Envoy’s global rate limit filter.
   ...
   ```sh
   kubectl apply -f manifests/global-ratelimit/filter-ratelimit.yaml
   ```
...
2. Apply another EnvoyFilter to the ingressgateway that defines the route configuration on which to rate limit. 
   ...
   * The configuration adds rate limit actions for any route from a virtual host.
   ```sh
   kubectl apply -f manifests/global-ratelimit/filter-ratelimit-svc.yaml 
   ```
